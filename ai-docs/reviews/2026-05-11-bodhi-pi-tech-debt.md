...
| **E** Event system: generalize emitter, fill gaps | ✅ shipped (E + E.4 both done) | 8 new events shipped in batch 1 (A); **E.4** (`advertiseSlashable` refresh hook) shipped in batch 4: `pi.registerCommand` after session boot now fires `available_commands_update` on every loaded session; `ExtensionAPI.requestSlashableRefresh(sessionId)` exposed as the explicit escape hatch. `session.commands` re-merges from frozen `projectCommands` + runner's current registry, keeping the slash resolver and the wire in lockstep. |
| **F** ACP extension-method catalog hygiene | ✅ shipped (batch 4) | F.1 `agentCapabilities._meta["bodhi-pi"]` collapses to `{ version: BODHI_PI_VERSION }`; F.2 `EXT_SESSION_CONFIG` response slims to 9 + 2 optional parse-error fields, per-scope layers move to `_bodhi-pi/session/settings/list`. Five host `/config` consumers + the http typed wrapper + http integration test migrated in lockstep. Kickoff: `ai-docs/reviews/kickoff-batch-4-acp-hygiene-and-slashable-refresh.md`. |
| **G** Dead code (`LabelEntry`) | ✅ shipped (with A) | Removed from union, exports, downstream imports. |
| **H** Tests + e2e gaps | 🔜 **next** | H.1/H.2/H.3; the settings/kv e2e in H.3 are unblocked (notifications shipped in batch 1, slashable refresh shipped in batch 4). Kickoff TBD. |
...
**Batch 4 verification:** All `bodhi-pi` unit + e2e green (344 unit + commands.e2e + extensions.e2e); 8 downstream hosts green per host (`bodhi-pi-node` 41, `bodhi-pi-cli` 22 + 31 e2e, `bodhi-pi-browser` 42, `bodhi-pi-http` 69, `bodhi-pi-ws-server` 36; web/chrome-ext/ws-frontend tsc -b clean) and under `just test`; `npm run check` clean. One `just test` flake on `bodhi-pi-cli` e2e (parallel-resource contention) re-passed standalone — same pattern as batch 3.
...
| 4 | F (capability advertisement + `EXT_SESSION_CONFIG` slimdown) + E.4 (`advertiseSlashable` refresh hook) | ✅ shipped | `ai-docs/reviews/kickoff-batch-4-acp-hygiene-and-slashable-refresh.md` |
| 5 | H (test helper extraction + e2e gap fills, including settings/kv now unblocked by batch 1) | 🔜 **next** | `ai-docs/reviews/kickoff-batch-5-test-helpers-and-e2e-gaps.md` |
...
- **F.1 capabilities `_meta` = single `version` stamp** (batch 4, locked 2026-05-12): `agentCapabilities._meta["bodhi-pi"] = { version: BODHI_PI_VERSION }`. CHANGELOG-as-contract: "if you see `version: <x.y.z>`, every `_bodhi-pi/*` method listed in this package's CHANGELOG for that version is present." Drops the seven mid-state keys (`sessionDelete`, `sessionCompact`, `sessionConfig`, `extensions.{tools,commands,providers,events}`) — they advertised only 7 of 17 implemented methods, which is worse than no advertisement at all. Wire-minimal; matches the PoC stance.
- **F.2 host migration = clean break, no warn-then-drop** (batch 4, locked 2026-05-12): every host migrates in the same release. Same precedent as batch 1's `configOptions` clean break. Hosts that print per-scope settings switch to `_bodhi-pi/session/settings/list?scope={…}`. No backward-compat dual-write.
- **E.4 = implicit + explicit refresh** (batch 4, locked 2026-05-12): implicit — `registerCommand`'s closure fires `void self.requestSlashableRefresh?.()` for every loaded session. Explicit — `pi.requestSlashableRefresh(sessionId)` on `ExtensionAPI` as a documented escape hatch for extensions whose registry mutations don't go through `registerCommand` directly. Both, not either-or, because implicit covers the 95% case while explicit covers templating-driven extensions that mutate their own registry under the hood.
- **E.4 = no debouncing, ship trailing-fire per registerCommand** (batch 4, locked 2026-05-12): five `registerCommand` calls produce five `available_commands_update` notifications. Add debouncing only if Playwright tests show observable host re-render churn. Premature.
- **E.4 `session.commands` refreshed alongside the wire** (batch 4, locked 2026-05-12): when `registerCommand` fires post-boot, `session.commands` (the snapshot used by the slash-command resolver at `agent.ts:1354`) AND the wire advertisement (`agent.ts:2046`) update in lockstep via `BodhiPiAgent.refreshSlashable(sessionId?)`. Added a `projectCommands: PromptTemplate[]` field to `SessionState` to make this re-merge possible (the boot-time project commands don't change at runtime). Otherwise `/help` would show a command that the resolver couldn't expand — a real "advertise what you don't deliver" bug.
- **Batch 4 commit cadence = three commits, one per concern** (batch 4 plan, locked 2026-05-12): F.1 (smallest blast radius — single `_meta` change) → E.4 (independent of F.1/F.2 — runner + agent + 5 tests) → F.2 (largest blast radius — slim + 5 host consumers + 1 typed wrapper + 1 http integration test). Each commit ends with bodhi-pi tests green → downstream rebuilt → `just test` green → `npm run check` clean. Smaller commits mean faster bisect if something regresses.
