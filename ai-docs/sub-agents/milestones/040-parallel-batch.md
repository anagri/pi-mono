# Milestone 040 — Parallel batch (P2b)

> ⚠ **Phase 1 in flight (2026-05-19)** — the `subagent_batch` LLM tool has been **unregistered** from the model's tool list as part of an empirical re-evaluation of [Decision 3](005-architecture-decisions.md#decision-3--two-llm-tools-not-one). The source code (`src/tools/subagent-batch.ts`, `src/subagents/batch-progress-accumulator.ts`, `SubagentService.spawnBatch`) is still present but no longer LLM-callable. Concurrency is now achieved by the LLM emitting multiple `subagent` tool calls in one assistant turn — pi-agent-core dispatches them in parallel via `Promise.all`. If Phase 1 evidence holds (load-bearing integration test in `test/subagents-parallel-tool-calls.test.ts`, e2e + e2e-ui replacements), Phase 2 will delete the batch surface entirely and mark this milestone Superseded. See [`ai-docs/plans/we-want-to-merge-jiggly-meteor.md`](../../plans/we-want-to-merge-jiggly-meteor.md). The historical record below describes what shipped in P2b — leave intact.
>
> **Status:** ☑ shipped (P2b phase, 2026-05-18 — seven commits, ending the same-day implementation arc).
