...
- **Two LLM tools:** `subagent` (single child) and `subagent_batch` (2–N concurrent children).
...
- **Three session-entry types:** `subagent_link`, `subagent_complete`, `subagent_batch` — durable in `SessionStore`.
- **Four lifecycle events** on `BodhiPiEvent`: `subagent_start`, `subagent_end`, `subagent_batch_start`, `subagent_batch_end`.
...
| 080 | Recursion opt-in + batch/single tool consolidation | Mastra (recursion); cc, Mastra (single tool covers N) | Conservative default (hard depth-cap-2, separate tools); reconsider once demand surfaces |
...
| Invocation interface | tool ↔ slash ↔ workflow node | **Tool** (`subagent`, `subagent_batch`) primary; flat `/agents` + `/subagent <name> <task>` as host-side affordance |
| Lifecycle | foreground ↔ background resume ↔ parallel batch | **Foreground + parallel batch**; background + resume deferred to milestones 050/060 |
...
| # | Title | Status | Brief |
|---|---|---|---|
| [005](005-architecture-decisions.md) | **Architecture decisions (READ FIRST)** | locked | Locks in-process spawn, profile-as-source-of-truth, separate `subagent`/`subagent_batch` tools, fresh-context default, depth-cap-2, MCP-empty-for-children, full-transcript fork filter. Not an implementation milestone. |
...
| [040](040-parallel-batch.md) | Parallel batch (P2b) | ☑ shipped, ◐ Phase 1 retirement in flight | `subagent_batch` tool (N≥2), `SubagentService.spawnBatch`, `BatchProgressAccumulator`, `failFast` + collect-all semantics. **Phase 1 (2026-05-19): the LLM tool is unregistered** pending Phase 2 deletion. Concurrency now flows through parallel `subagent` tool calls. See [`ai-docs/plans/we-want-to-merge-jiggly-meteor.md`](../../plans/we-want-to-merge-jiggly-meteor.md). |
...
| [080](080-recursion-and-tool-consolidation.md) | Recursion opt-in + tool consolidation | ☐ pending | Opt-in recursion past the hard depth-cap of 2; fold `subagent_batch` into `subagent` (drop `minItems: 2`). |
...
| [100](100-advanced-slash-ux.md) | Advanced slash UX | ☐ pending | `/run`, `/chain`, `/parallel` and friends beyond the flat `/agents` + `/subagent`. Cross-host parity required. |
