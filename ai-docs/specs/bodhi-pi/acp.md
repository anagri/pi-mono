...
- `{type:"subagent_start", parentSessionId, childSessionId, profileName, task, toolCallId, depth, contextMode}` — emitted when `SubagentService.spawn` creates a child session and is about to run its prompt loop. `profileName` matches the persisted `subagent_link.profileName` + the SessionStore `subagent.profileName` shape so all rails name the field the same. `contextMode: "fresh" \| "fork"` mirrors the profile's `context` field so clients know whether the child inherited the parent transcript. Lets clients open a transcript group keyed by `childSessionId` and route subsequent `sessionUpdate` notifications whose `sessionId === childSessionId` into that group.
- `{type:"subagent_end", parentSessionId, childSessionId, profileName, status: "completed" \| "cancelled" \| "failed", durationMs, toolCount, contextMode, summary?, error?}` — emitted after the child's prompt loop terminates (regardless of status) and before the `evictChild` cleanup. `contextMode` matches the corresponding `subagent_start`. Lets clients close their open transcript group and record the terminal status.
- `{type:"subagent_batch_start", parentSessionId, batchToolCallId, childSessionIds: string[], profileNames: string[], tasks: string[], failFast: boolean}` — emitted when `SubagentService.spawnBatch` pre-allocates the N children and is about to dispatch them in parallel. `childSessionIds[]`/`profileNames[]`/`tasks[]` are zero-aligned with the parent's input order so Hosts can pre-allocate per-child UI slots. Per-child `subagent_start` events still fire underneath this envelope. `batchToolCallId` is the parent's tool call id for the `subagent_batch` invocation.
- `{type:"subagent_batch_end", parentSessionId, batchToolCallId, childSessionIds: string[], profileNames: string[], statuses: ("completed"|"cancelled"|"failed")[], durationMs}` — emitted after every child settles (collect-all) or after fail-fast cancels in-flight siblings. `statuses[]` is zero-aligned with `childSessionIds[]`. Per-child `subagent_end` events still fire; the envelope is for batch-level UI grouping/aggregation.
