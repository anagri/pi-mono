...
- `{type:"tool_blocked", sessionId, toolCallId, toolName, category, mode, reason, serverTime}` — emitted when `PermissionService.evaluateToolCall` returns `{ kind: "deny" }` and `createPiAgent.beforeToolCall` rejects the call. One-shot (no wire round-trip — no `correlationId`). The agent ALSO appends a `custom_message` SessionEntry (`extensionName: "modes"`, `customType: "tool_blocked"`, `content: reason`, `display: true`) so the block joins the persisted transcript and the LLM's next context window. Clients should render the lifecycle event live (the persisted entry is for replay on load). Plan-mode denials fire here always; ask-mode `reject_*` / cancel / timeout verdicts also surface here in addition to `tool_approval_response`.
- `{type:"tool_approval_request", sessionId, correlationId, toolCallId, toolName, category, pattern, timeoutMs, serverTime}` — emitted (milestone 040) when an ask-category tool call enters the approval round-trip, just before the agent issues the native `session/request_permission`. `correlationId` pairs it with the matching `tool_approval_response`; `pattern` is the toolName (a glob in milestone 100). The agent SHOULD have already emitted the `tool_call` notification (`status: "pending"`) for the same `toolCallId` so Clients can render the pending card before the prompt.
- `{type:"tool_approval_response", sessionId, correlationId, toolCallId, toolName, kind, serverTime}` — emitted (milestone 040) once the round-trip resolves. `kind ∈ {allow_once, allow_always, reject_once, reject_always, cancelled, timeout}` records WHAT the user picked (or how it ended). `allow_*` lets the tool run; the other verdicts ALSO produce a `tool_blocked` (the gate's denial). `*_always` records an in-memory per-session grant (`runtime.permissionGrants`). These two events are wire-forwarded purely for observability — the actual decision rides `session/request_permission`, not the notification.
