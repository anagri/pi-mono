...
// Cross-provider half is opt-in: only runs if ANTHROPIC_API_KEY is in
// e2e/.env.test. Mirrors bodhi-pi/e2e/chat.e2e.ts "switching model mid-session"
// — same provenance signal, same setSessionConfigOption flow, but driven through
// the Node host (createCliAgent + createSqliteSessionStore + createNodeFilesystem).
const skipCrossProvider = !ANTHROPIC_KEY;

test.skipIf(skipCrossProvider)(
	"mid-session model switch via setSessionConfigOption flips OpenAI ↔ Anthropic provenance",
	async () => {
