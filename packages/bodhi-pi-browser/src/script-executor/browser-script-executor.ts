...
/**
 * Browser-only `ScriptExecutor` that wraps the script body in an
 * `AsyncFunction` and runs it on the calling thread (i.e. inside the worker
 * that hosts the agent). Console writes are captured into stdout/stderr;
 * thrown errors land in stderr with exitCode=1. Optional timeout races a
 * setTimeout reject.
 *
 * Same trust posture as the bodhi-pi-cli Node executor — scripts come from
 * project disk, no extra sandbox. Browser hosts that need stricter isolation
 * can spawn a nested Worker with a Blob URL; left as a follow-up.
 */
