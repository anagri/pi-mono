...
/**
 * Hand-rolled flag parser. No dependency on `commander` etc. Mirrors the shape
 * used by `bodhi-pi-cli/src/config.ts`. Throws on unknown flags so typos surface
...
function helpText(): string {
	return `bodhi-pi-ws-server — WebSocket-hosted multi-user agent

Usage: bodhi-pi-ws-server [options]

Options:
  --port <n>            TCP port to bind. Use 0 for a random free port. Default: env PORT or 8788.
