...
/**
 * Upsert a user row. Called from the WS upgrade handler when a connection is accepted.
 * Email is overwritten on each connect so the latest claim wins (acceptable for PoC; the
 * fake auth has no signing so users can already self-serve).
 */
 ...
 * Cross-tenant access is silent on `load`/`list` (returns undefined / empty)
 * and throws on `append`/`delete`.
 */
