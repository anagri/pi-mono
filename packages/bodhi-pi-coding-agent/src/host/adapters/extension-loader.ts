...
export async function createNodePackageExtensionLoader(opts: NodePackageLoaderOptions): Promise<RegisteredExtension[]> {
	...
	for (const { name, isDir } of entries.sort((a, b) => a.name.localeCompare(b.name))) {
		...
		if (seen.has(extensionName)) {
			console.warn(`[extension-loader] name collision: "${extensionName}" already loaded; skipping ${name}`);
			continue;
		}
...
async function loadDirectoryExtension(
	...
	try {
		...
		if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
			console.error(`[extension-loader] failed to read ${pkgPath}:`, err);
		}
	}
	...
	console.warn(`[extension-loader] no loadable entry found for extension directory ${dir}`);
	return undefined;
}
...
async function loadModuleDefault(
	...
	try {
		...
		if (typeof mod !== "function") {
			console.warn(`[extension-loader] ${filePath}: default export is not a function — skipping`);
			return undefined;
		}
		return mod as ExtensionFactory;
	} catch (err) {
		console.error(`[extension-loader] failed to load ${filePath}:`, err);
		return undefined;
	}
}
