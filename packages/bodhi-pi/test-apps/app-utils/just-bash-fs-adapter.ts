...
/**
 * Wrap a bodhi-pi `Filesystem` so it satisfies just-bash's `IFileSystem`.
 *
 * The adapter is runtime-neutral — it depends only on bodhi-pi's `Filesystem`
 * surface plus standard `TextEncoder`/`TextDecoder`/`posix.resolve`. Both
 * `test-app-http` (`just-bash`) and `test-app-browser` (`just-bash/browser`)
 * consume it.
 *
 * Methods bodhi-pi's Filesystem doesn't expose directly:
 *   - `cp`/`mv` — implemented as compound ops over read/write/stat/list/remove.
 *   - `chmod`/`symlink`/`link`/`readlink`/`utimes` — silently swallowed
 *     (web-acp-agent precedent); bash commands like `chmod +x` don't surface
 *     a fatal error on platforms that can't honour them.
 *   - `lstat`/`realpath` — fall through to stat / return the input path.
 *   - `getAllPaths` — returns `[]`; bash's glob layer falls back to per-dir
 *     `readdir` traversal when this is empty.
 */
export function createJustBashFsAdapter(filesystem: Filesystem): IFileSystem {
	...
	const adapter: IFileSystem = {
		...
		async chmod() {
			// no-op — bodhi-pi Filesystem has no permission model.
		},
		async symlink() {
		...
		async utimes() {
			// no-op — bodhi-pi Filesystem doesn't expose utimes.
		},
	};
	return adapter;
}
