...
import { bindFsBridge } from "@bodhiapp/bodhi-pi-test-app-browser/lib/worker-fs-bridge";
import { WORKSPACE_NAME, WORKSPACE_ROOT } from "@bodhiapp/bodhi-pi-test-app-browser/lib/workspace-constants";
import type { WorkerMessage } from "@bodhiapp/bodhi-pi-test-app-browser/runtime/types";
import { createMessagePortStream } from "@bodhiapp/bodhi-pi-test-app-browser/transport/message-port-stream";
import type {
	ConnectCallbacks,
	ConnectResult,
	SetupFormValues,
...
export function createChromeExtAdapter(): TransportAdapter {
	...
	return {
		async connect(values: SetupFormValues, callbacks: ConnectCallbacks): Promise<ConnectResult> {
			...
			// MV3 extension_pages CSP forbids unsafe-eval in the worker realm;
			// AsyncFunction / run-script execution rides through the sandbox iframe.
