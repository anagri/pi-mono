...
// bash tool is wired into:
//   - test-app-cli  -> child_process.spawn('bash', ['-c', cmd])  (real shell)
//   - test-app-http -> just-bash (node)  (sandboxed shell; covers http + ws projects)
//   - test-app-browser / chrome-ext -> just-bash/browser  (sandboxed browser shell)
// The in-memory runtime has no terminal wired — the `bash` tool isn't registered there.
const harness = useHarness();

test.runIf(!isRuntime("in-memory"))(
	"bash tool (gpt-4o-mini): echo, pwd, pipeline, and shared-fs round-trip",
	async () => {
