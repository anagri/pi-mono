...
import type { Filesystem, Terminal, TerminalExecInput, TerminalExecResult } from "@/index.js";
...
/**
 * Lockstep copy of `test-apps/app-utils/just-bash-terminal.ts` (e2e is a blackbox
 * suite that may not import test-app packages). Builds a bodhi-pi `Terminal` backed
 * by a just-bash `Bash` shell over the injected `Filesystem`, so the in-memory e2e
 * runtime can exercise the `bash` tool with no real shell.
 */
