...
export function App() {
	...
	const onSetupSubmit = useCallback(
		async (e: React.FormEvent) => {
			...
			try {
				...
				// Boot the sandbox iframe BEFORE creating the worker. MV3
				// extension_pages CSP forbids unsafe-eval in the worker realm;
				// AsyncFunction/run-script execution rides through the sandbox
				// iframe's relaxed CSP. cached across the session.
	...
	return (
		<main data-testid="test-app-root" data-test-state={state}>
			<h1>bodhi-pi test-app-chrome-ext</h1>
			{state === "error" && (
