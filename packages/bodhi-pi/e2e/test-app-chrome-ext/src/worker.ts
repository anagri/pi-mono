...
// under `e2e/app-utils/browser/`. Init message carries the seedFiles +
// adapter config + sandboxPort; `bootstrapAgentWorker` mounts an InMemory
// ZenFS at `cwd`, swaps in the sandboxed script-executor / extension-loader
// variants (MV3 forbids unsafe-eval in the worker realm), then wires Dexie
// session/kv stores.
//
...
import { bootstrapAgentWorker } from "@e2e/app-utils/browser/runtime/bootstrap-worker";
