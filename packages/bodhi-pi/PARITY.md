...
session capability and how bodhi-pi covers it across the four reference Hosts
(`test-apps/cli`, `test-apps/http`, `test-apps/browser`, `test-apps/chrome-ext`).
...
| Pagination cursor in `session/list` | (already in ACP) | ✅ | All four stores encode `{updatedAt,id}` base64url cursors with `LIMIT PAGE_SIZE+1` over-fetch. Each Host's `/sessions` slash dispatcher loops until `nextCursor` is undefined. |
...
| Per-session resolved config | `_bodhi-pi/session/config` | ✅ | Returns `{ sessionId, cwd, defaultModelId, currentModelId, thinkingLevel, retryOptions, compaction, appendSystemPrompt, contextFilePaths }` plus optional `globalSettingsParseError` / `projectSettingsParseError`. Per-scope settings layers (`global`/`project`/`session`/`effective`) live on `_bodhi-pi/session/settings/list` and are not duplicated here. Per-host `/config` slash command renders this for visual blackbox verification. |
...
| Host-injected `KvStore` + auth credential storage | `BodhiPiConfig.kvStore` + `_bodhi-pi/kv/{get,set,list,remove}` | ✅ | Phase I. Generic key-value primitive with `secret: boolean` hint. API keys stored under `auth/<provider>`. **ACP reads mask secret values to `***`**; internal in-process resolution (`getApiKey` path) reads unmasked. Adapters: `createNodeKvStore` (file-per-key, 0o600 for secrets), `createDexieKvStore` (two-table segregation: `kv` + `kv_secret`). Wired into all four reference Hosts (`test-apps/{cli,http,browser,chrome-ext}`). Tests: `bodhi-pi/test/kv-store.test.ts`, `bodhi-pi/test/kv-slash.test.ts`. |
| Flat slash commands: `/settings`, `/login`, `/logout`, `/logins` | (Host dispatchers) | ✅ | Phase I. All four reference Hosts (`test-apps/{cli,http,browser,chrome-ext}`) ship word-for-word identical surface. `/settings list|get|set|unset [--global|--project|--session]`. `/login <provider> <api-key>` → `_bodhi-pi/kv/set auth/<provider> <key> secret:true`. `/logout <provider>`. `/logins` lists stored providers with masked values. Flat-and-complete by design — no prompts, no popups, no UI cycle (UI can implement cycling natively over ACP). `--global` errors with `-32602` when Host has no `homeDir`. Tests: `bodhi-pi/test/settings-slash.test.ts`. |
...
| Dynamic model registry (pi-ai catalog filtered by stored auth) | (`allModels()` in `BodhiPiAcpAgent`) | ✅ | Phase J. Agent core derives the model list from pi-ai's `getProviders()` + `getModels(provider)`, filtered by `resolveProviderApiKey(provider) !== undefined`. Host-supplied `BodhiPiConfig.models` are additive — when the host lists ANY model for a provider, the catalog for that provider is suppressed (tests/local-LLMs win). `/login <provider> <key>` writes to kvStore; the next `_bodhi-pi/session/config` reflects the new models. `BodhiPiConfig.models`, `defaultModelId`, `getApiKey` are now all optional. |
| Zero production env-var API-key reading | (n/a — by absence) | ✅ | Phase J. No reference Host reads `OPENAI_API_KEY`/`ANTHROPIC_API_KEY`/`VITE_*_API_KEY` in production code. `dotenv` dropped from `test-apps/cli`, `test-apps/http`. `test-apps/browser` and `test-apps/chrome-ext` no longer bake `VITE_*` keys into the static bundle. All auth flows through `/login <provider> <key>` → `_bodhi-pi/kv/set auth/<provider>` → Host-injected `KvStore`. Tests load real keys from per-package `e2e/.env.test` and seed via the same ACP path (helper: `test/helpers/seed-auth.ts`'s `seedAuth(...)` runs `/login` → `/settings set defaultModel ... --project` → `/model <id>`). |
| `defaultModel` project-settings key | `_bodhi-pi/session/settings/set` (existing) | ✅ | Phase J. New `BodhiPiProjectSettings.defaultModel` field. `pickDefaultModelId` consults explicit `BodhiPiConfig.defaultModelId`, then merged `defaultModel` from settings, then first auth-available model. Tests use `/settings set defaultModel <id> --project` for blackbox setup. |
...
| OAuth login flow (Anthropic Claude.ai et al.) + OAuth token refresh | Phase I shipped API-key persistence via the generic `KvStore`. The first-cut OAuth-DCR implementation landed and was then removed (commit `6a3966f4`) because it lacked end-to-end coverage across the runtime matrix. Re-introduction is tracked in `ai-docs/prompts/bodhi-pi-mcp-auth-oauth-{dcr,preregistered,header-query}.md`. Only `auth.mode = "public"` is supported today. |
| `models.json` user-editable catalog + `modelOverrides` + OAuth `modifyModels` | Phase J shipped the **simple half** of dynamic registry: pi-ai catalog filtered by stored auth. Still deferred: user-editable `~/.bodhi-pi/models.json` for custom providers and the OAuth `modifyModels` post-login callback. Both land alongside OAuth re-introduction. |
...
| Sub-agents (`.claude/agents/`) | Shipped as bodhi-pi sub-agents: project markdown under `<cwd>/.bodhi-pi/agents/<name>.md` + bundled built-ins (`explore`, `planner`) + extension-registered via `ExtensionAPI.registerSubagentProfile`. Parity across cli + http (per-turn agent rebuild) + browser (Web Worker) + chrome-ext (MV3). Supports `context: "fresh" \| "fork"` (fork inherits a filtered parent-transcript slice). See [ai-docs/specs/bodhi-pi/subagents.md](ai-docs/specs/bodhi-pi/subagents.md). |
