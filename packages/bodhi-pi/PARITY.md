...
| Zero production env-var API-key reading | (n/a — by absence) | ✅ | Phase J. No host reads `OPENAI_API_KEY`/`ANTHROPIC_API_KEY`/`VITE_*_API_KEY` in production code. `dotenv` dropped from `bodhi-pi-cli`, `bodhi-pi-http`, `bodhi-pi-ws-server`. `bodhi-pi-web` and `bodhi-pi-chrome-ext` no longer bake `VITE_*` keys into the static bundle. All auth flows through `/login <provider> <key>` → `_bodhi-pi/kv/set auth/<provider>` → host-injected `KvStore`. Tests load real keys from per-package `e2e/.env.test` and seed via the same ACP path (helper: `test/helpers/seed-auth.ts`'s `seedAuth(...)` runs `/login` → `/settings set defaultModel ... --project` → `/model <id>`). |
