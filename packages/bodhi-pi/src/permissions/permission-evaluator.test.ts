...
describe("PermissionService.evaluateToolCall — plan mode", () => {
	it("allows read tool", async () => {
		...
		const decision = await service.evaluateToolCall(sessionId, { name: "read", arguments: {} });
	...
	it("allows search tools (ls, find, grep)", async () => {
		...
		for (const name of ["ls", "find", "grep"]) {
			const d = await service.evaluateToolCall(sessionId, { name, arguments: {} });
	...
	it("allows subagent", async () => {
		const { service, sessionId } = buildService("plan");
		const decision = await service.evaluateToolCall(sessionId, { name: "subagent", arguments: {} });
		expect(decision).toEqual({ kind: "allow" });
	});
	...
	it("denies unknown 'other' tools defensively", async () => {
		const { service, sessionId } = buildService("plan");
		const d = await service.evaluateToolCall(sessionId, { name: "totally_unknown_tool", arguments: {} });
		expect(d.kind).toBe("deny");
	});

	it("deny reason follows the redirect template with toolName + category + /mode edit hint", async () => {
		const { service, sessionId } = buildService("plan");
		const d = await service.evaluateToolCall(sessionId, { name: "write", arguments: {} });
		expect(d.kind).toBe("deny");
	...
	it("allows MCP tool with readOnlyHint=true", async () => {
		...
		const d = await service.evaluateToolCall(sessionId, { name: "github__list_issues", arguments: {} });
	...
	it("denies MCP tool with destructiveHint=true", async () => {
		...
		const d = await service.evaluateToolCall(sessionId, { name: "github__delete_repo", arguments: {} });
		expect(d.kind).toBe("deny");
	});
	...
	it("allows MCP tool with no annotations (research-permissive default)", async () => {
		...
		const d = await service.evaluateToolCall(sessionId, { name: "github__unknown_tool", arguments: {} });
		expect(d).toEqual({ kind: "allow" });
	});
...
describe("MODE_PRESETS.plan", () => {
	...
	it("leaves ask/edit/allow-all/policy untouched (phase 1 only fills plan)", () => {
